...
from app.utils.helpers import json_loader, choice_list, check_characters
...
def autosuggest():
    ...
    query = Model.query.filter_by(user_id=current_user.id).filter(Model.name.ilike(f'{value}%'))\
        .with_entities(Model.name).distinct()
...
def usersettings():
    ...
    # If the setting to be updated is "query_currency"
    if setting_name == 'query_currency':
        # Load all possible currency query types from the JSON file and create a list with those options, in order to
        # check if the option/value sent in the request exists as a standard option for the currency querying
    ...
    if form.validate_on_submit():
        ...
        if sibling_items.first().category != category.name:
            for item in sibling_items.all():
                ...
                db.session.add(item)
        db.session.commit()
        # Inform the user on successful entry if the item name does not contain special characters
        if check_characters(item.name):
            flash(f'Item {item.name} was added')
        # If the item price is not entered in the default/base currency for the user, the second price in the main/base
        # currency also gets defined here. A Celery background task sends an api request to a website
        # that returns that price converted to the base/main currency, with the exchange rate for the specified date
        # The Celery function is used for looping over all the results received in order to enter them into a database,
        # and since in this case the request/result is singular, it still needs to be placed inside a list because the
        # for loop expects a list
        currency = form.currency.data
        if currency != base_currency:       # If the price was not entered in the default currency for the user
            # The Celery function uses the item_id in order to uniquely identify the item for which the price should
            # be entered. Due to the fact that the item that was just entered only received an ID upon being committed
            # to the database, the query below is used to instantly return that item, so that the Celery background
            # task could use the newly defined ID for that item in order to add the price in another currency for it
...
def item_edit(username, item, item_id):
    ...
    form = ItemForm()
    # Populate the select field with the relevant currency as the first/active choice
    ...
    if request.method == 'POST':                    # If the user clicked any of the three submit buttons
        ...
        if requested['submit'] == 'Cancel':         # If no changes were made - redirect the user to the overview page
            ...
            if form.validate_on_submit():
                ...
                # Update the category name if its name differs from the original category name for that item, and
                ...
                if price.price != form.price.data or price.currency != form.currency.data:
                    ...
                    db.session.commit()         # Commit the changes to the current price
                    # Inform the user on successful edit if the item name does not contain special characters
                    if check_characters(item.name):
                        flash(f'Changes made to the item "{item.name}"')
                    # If the currency for the entry differs from the default/base currency for that user, a conversion
                    ...
                    if price.currency != base_currency:
                        price_data= [{
                            ...
                            'entry_currency': form.currency.data,
                            'target_currency': base_currency,
                            'first_entry': False    # "First_entry" is "False", since this is the second price for the
                        }]                          # item - i.e. original entry converted to the main/base currency
    ...
    return render_template('edit_entries.html', title='edit entry', form=form)
...
def overview():
    ...
    # Else send an empty object string, which when evaluated does not trigger the display of the most recent query
    else:
        last_query = '{}'
    # The "save_query" option being active renders "temp_query" option superfluous
    ...
    return render_template(
        ...
        last_query=json.loads(last_query)
    )
...
def profile(username):                          # Argument only for the URL, not used in the view function
    ...
    if request.method == 'POST':
        ...
        if submit_button == 'submit-password':                  # If the form for changing the password was submitted
            form = EditUserPasswordForm(current_user.username)      # instantiate it with current_user in order
            form.old_password.data = form_data['old_password']      # to be able to validate the old/current password
            ...
            if form.validate_on_submit():
                if user.check_password(password) != True:   # Check if the user typed the old password as the new  #TODO
                    ...
                    return {'data': 'updated'}              # already validated (superfluous step most likely)
                else:
                    return {'data': 'no change'}
            else:
        ...
        if submit_button == 'submit-settings':
            ...
            base_currency = user.setting('base_currency')  # Get the current base currency
            ...
            if form.validate_on_submit():
                ...
                if user.setting('save_query') != form.save_query.data or base_currency != form.currency.data:
                    ...
                    if base_currency != form.currency.data:
                        ...
                        # Update currency setting after the prices are committed to the database
