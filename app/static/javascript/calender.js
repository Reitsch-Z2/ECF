...
function generateCalender(
  ...
  */
  ...
  for (let i = 0; i < content.weeks.length; i++) {  // Gor each week create a table row, and populate it via the
    ...
    row.classList.add('week')                       // of calls to the dayLooper function that gets called once or twice
    body.append(row)
...
function timeMarker() {
  ...
  cal.addEventListener('click', function(e) {                     // TODO restructure it as a switch in the future?
    if (typeof mode == 'undefined'){
      ...
      /*
      The "additional" argument used in datePipeline function (only for the weeks) - if the "outer days" (from past or
...
function datePipeline(Y, M, D, mode, additional=0) {      // Function handling the return of proper date formats for
  switch(mode) {                                          // all three query modes.
    ...
    case 'month':
      ...
      break
    case 'period':      // Option to be added in the future
      // pass
      break
  }
}
...
function queryUnpacker(query) {
  ...
  */
  ...
  if (Object.keys(query.query_type).length != 0) {    // If the user chose the Item/Category filter, it gets set up
    let qtype = query.query_type                      // here again - based on the previous-query parameters
